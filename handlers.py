import os
import requests
PATH = os.getenv('SERVER_IP_PATH')


def create_user(user):
    req = requests.post(f'{PATH}/users',
                        json=user)
    return req.status_code


def get_user(user_id):
    req = requests.get(f'{PATH}/users/{user_id}')
    return req.json()


def get_user_dashboards(chat_id):
    req = requests.get(
        f'{PATH}/users/{chat_id}/data?query=dashboards')
    return req.json()


def get_user_stats(chat_id, query_parameter):
    req = requests.get(f'{PATH}/users/{chat_id}'
                       f'/data?query={query_parameter}')
    return req.json()


def get_user_by_email(query_parameter):
    req = requests.get(f'{PATH}/users?email={query_parameter}')
    return req.json()


def get_user_dashboards_as_admin(chat_id):
    req = requests.get(
        f'{PATH}/users/{chat_id}/dashboards')
    return req.json()


def update_user(chat_id, data):
    req = requests.patch(
        f'{PATH}/users/{chat_id}',
        json=data
    )
    return req.status_code
...
def create_dashboard(chat_id, data):
    req = requests.post(
        f'{PATH}/users/{chat_id}/dashboards', json=data)
    return req.status_code


def get_dashboard(d_id):
    req = requests.get(
        f'{PATH}/dashboards/{d_id}')
    return req.json()


def get_dashboard_users(d_id):
    req = requests.get(
        f'{PATH}/dashboards/{d_id}/users')
    return req.json()


def get_dashboard_tasks(d_id):
    req = requests.get(
        f'{PATH}/dashboards/{d_id}/tasks')
    return req.json()


def get_dashboard_stats(d_id):
    req = requests.get(
        f'{PATH}/dashboards/{d_id}/data')
    return req.json()


def delete_dashboard(chat_id, d_id):
    req = requests.delete(
        f'{PATH}/users/{chat_id}/dashboards/{d_id}')
    return req.status_code
...
def add_user_to_dashboard(admin_id, user, d_board_id):
    req = requests.post(
        f'{PATH}/users/{admin_id}/dashboards/{d_board_id}',
        json={"team": user})
    return req.status_code
...
def remove_user_from_dashboard(d_board_id, user_id):
    req = requests.delete(
        f'{PATH}/dashboards/{d_board_id}/users/{user_id}')
    return req.status_code
...
def get_task(chat_id, d_id, t_id):
    req = requests.get(
        f'{PATH}/users/{chat_id}/dashboards/{d_id}/tasks/{t_id}')
    return req.json()


def get_task_users(t_id):
    req = requests.get(
        f'{PATH}/tasks/{t_id}/users')
    return req.json()


def get_task_all_comments(task_id):
    req = requests.get(f'{PATH}/tasks/{task_id}/comments')
    return req.json()


def add_user_to_task(initiator_id, user_id, dashboard_id, task_id):
    req = requests.post(
        f'{PATH}/users/{initiator_id}/dashboards'
...
def get_comment(comment_id):
    req = requests.get(f'{PATH}/comments/{comment_id}')
    return req.json()


def post_comment(comment_data):
    req = requests.post(
        f'{PATH}/users/{comment_data["chat_id"]}/dashboards'
...
def get_user_comments(chat_id):
    req = requests.get(
        f'{PATH}/users/{chat_id}/data?query=comments')
    return req.json()
