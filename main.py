...
@bot.message_handler(func=lambda x: x.text == 'Main Menu')
...
def get_tasks(message):
    ...
    print(tasks)
    tasks_hidden = [t.get('id') for t in tasks]
...
def update_task(message, task_data, status_buttons=None):
    ...
    if task_data[
        'update_instance'] == 'status' and message.text not in status_buttons:
        msg = bot.send_message(message.chat.id,
                               'Wrong input. Please choose from the following '
                               'options:')
        bot.register_next_step_handler(msg, update_task, task_data,
